...
/*hp define*/
const char *PATH_RECEIVE = "receive.h263";
...
static void dec_process_frame(MSFilter *f, mblk_t *inm){
	...
	if (inm){
		...
		if (mblk_get_marker_info(inm)){
			...
			frame=s->input;
			s->input=NULL;
			/*hp add record video for mesuarence quality*/
			//FILE *file_recvideo = fopen(PATH_RECEIVE, "w");
			//fclose(file_recvideo);
			while ( (remain=frame->b_wptr-frame->b_rptr)> 0) {
			  /*hp add record video for mesuarence quality*/
			  FILE *file_recvideo = fopen(PATH_RECEIVE,"a");
			  fwrite(frame->b_rptr, sizeof(unsigned char), remain, file_recvideo);
			  fflush(file_recvideo);
			  fclose(file_recvideo);
			  /*end add*/
				AVPacket pkt;
				av_init_packet(&pkt);
